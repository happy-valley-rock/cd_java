...
import com.pos.model.ProductPurchase;
...
public class ProductService {
    ...
    public ProductPurchase purchaseProduct(Integer productId, Integer quantity) {
        try {
            Product product = this.getById(productId);
            if (quantity > product.getStock()) throw new Error("Insufficient product stock");
            return new ProductPurchase(product);
        } catch (Exception exception) {
            exception.printStackTrace();
            throw exception;
        }
